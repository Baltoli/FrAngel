...
public class FrAngel {
  ...
  private int genCount, runCount, genAngelicCount, runAngelicCount, genNoAngelicCount,
      runNoAngelicCount;

  private long lastDebug, lastResolveConditions;

  public static void synthesize(SynthesisTask task) {
    synthesize(task, 1000000, Settings.VERBOSE);
  }
  ...
  public FrAngel(SynthesisTask task) {
    ...
    genCount = runCount = genAngelicCount = runAngelicCount = genNoAngelicCount =
        runNoAngelicCount = 0;
  }

  public FrAngelResult run(long timeout) {
    // Don't time this. SyPet doesn't include JAR parsing (about 7 sec) in their time. This is
    // FrAngel's equivalent. This typically takes ~0.2 sec
    ...
    DateTimeFormatter formatter =
        DateTimeFormatter.ofPattern("dd/MM/yy h:mm:ss a").withZone(ZoneId.systemDefault());

    while (!Utils.timeout(timeout)) {
      printDebugInfo();

      boolean useAngelic = Utils.randBoolean()
          ? false
          : Settings.USE_ANGELIC_CONDITIONS; // sometimes generate concrete conditions
      ...
      if (passed.cardinality() == numExamples) {
        ...
        if (good) {
          ...
          if (!ProgramUtils.getUsedVars(ans).containsAll(ans.getArgVars())) {
            System.err.println(
                "Deep-cleaning produced bad program (reverting to uncleaned version):\n"
                + ans.toJava());
            ans = p;
          }
          ...
          System.err.println(
              "Bad program for " + task.getName() + ", probably nondeterministic:\n" + p.toJava());
          continue;
        }
      }
    }

    return new FrAngelResult(
        this, ans, Utils.timeSince(start), unCleanedProgram, unCleanedProgramSize);
  }
      ...
      if (Settings.VERBOSE > 1) {
        if (Settings.MINE_FRAGMENTS) {
          ...
          for (Class<?> cls : expressionFragments.keySet()) {
            ...
            for (Expression e : expressionFragments.get(cls)) System.out.println("  " + e.toJava());
          }
          if (!statementFragments.isEmpty())
            System.out.println("\nStatement fragments:");
          for (Statement s : statementFragments) System.out.print("  " + s.toJava());
        }
        printCounts();
      }
    }
  ...
  public void printCounts() {
    ...
    System.out.println(
        "Only non-angelic: generated " + genNoAngelicCount + ", ran " + runNoAngelicCount);
    System.out.println(
        "Only angelic:     generated " + genAngelicCount + ", ran " + runAngelicCount);
  }
  ...
  private static void findCodePaths(
      int numTrue, StringBuilder path, List<String> paths, BitStringTrie used, int limit) {
  ...
  private boolean angelicCheck(Program p, int exampleNum) {
    ...
    for (int numTrue = 0; used.size() < Settings.NUM_ANGELIC_CODE_PATHS; numTrue++) {
      ...
      findCodePaths(
          numTrue, new StringBuilder(), paths, used, Settings.NUM_ANGELIC_CODE_PATHS - used.size());
      if (paths.isEmpty()) {
      ...
      for (String path : paths) {
        ...
        if (Settings.VERBOSE > 3)
          System.out.println("  path: " + path + ", actual: " + info.getActualCodePath()
  ...
  private BitSet resolveSingleCondition(
      Program p, AngelicStatementInfo info, BitSet passed, long timeout) {
    long start = System.nanoTime();
    ...
    if (rememberedCondition != null) {
      ...
      if (newPassed != null) {
        if (Settings.VERBOSE > 1) {
          System.out.printf(
              "Resolved with remembered condition: %s\n", rememberedCondition.toJava());
    ...
    for (String var : info.getInScopeVars()) p.addToScope(var);

    boolean triedTrue = false;
    boolean triedFalse = false;
    int i;
    for (i = 0; !Utils.timeout(timeout); i++) {
      Expression condition = p.getExpressionGenerator().genAnyExp(
          Utils.randInt(1, Settings.MAX_RESOLVE_COND_SIZE), boolean.class, false);
      ...
      if (newPassed != null) {
        if (Settings.VERBOSE > 1) {
          System.out.printf("Resolved a angelic condition (%d tries, %.3f sec): %s\n", i + 1,
              Utils.timeSince(start), condition.toJava());
          System.out.println("Now passes: " + newPassed);
        }
        for (String var : info.getInScopeVars()) p.removeFromScope(var);
        return newPassed;
      }
    }
    if (Settings.VERBOSE > 1)
  ...
  private void findAngelic(List<Statement> statements, int depth, Set<String> inScopeVars,
      List<AngelicStatementInfo> list) {
  ...
  private BitSet resolveConditions(Program p, BitSet passed, long timeout) {
    ...
    double singleTimeout = Math.max(Settings.MIN_RESOLVE_CONDITIONS_SEC,
        Math.min(2 * secSinceLast, Settings.MAX_RESOLVE_CONDITIONS_SEC));
    if (Settings.VERBOSE > 1)
    ...
    for (AngelicStatementInfo info : list) {
      passed = resolveSingleCondition(
          p, info, passed, Math.min(timeout, Utils.getTimeout(singleTimeout)));
      if (passed == null)
        break;
    }
    ...
    if (passed == null && list.size() > 1) {
      ...
      for (AngelicStatementInfo info : list) setCondition(info.getStatement(), null);
      passed = copy;
      list = findAngelic(p);
      Collections.reverse(list);
      for (AngelicStatementInfo info : list) {
        passed = resolveSingleCondition(
            p, info, passed, Math.min(timeout, Utils.getTimeout(singleTimeout / 2)));
        if (passed == null)
          break;
      }
    }
  ...
  private BitSet mineFragments(Program p, BitSet passed) {
    ...
    for (Map.Entry<BitSet, Program> entry : fragmentPrograms.entrySet()) {
      ...
      if (copy.equals(otherPassed)) { // other program passes (at least) the same examples
        ...
        if (otherSize < curSize
            || (otherSize == curSize && otherProgram.toJava().length() <= p.toJava().length())) {
    ...
    if (keep) {
      ...
      for (BitSet otherPassed : new HashSet<BitSet>(fragmentPrograms.keySet())) {
        ...
        BitSet copy = (BitSet) passed.clone();
        copy.or(otherPassed);
        if (copy.equals(passed)
            && ProgramUtils.size(fragmentPrograms.get(otherPassed)) >= ProgramUtils.size(p))

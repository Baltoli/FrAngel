...
public class Settings {
  ...
  public static SynthesisTask TASK = (TASK_CREATOR == null
          ? null
          : TASK_CREATOR.createTask().setGroup(BenchmarkGroup.findGroup(TASK_CREATOR).toString()));
  ...
  public static final boolean HARDCODE_POLYMORPHISM =
      true; // Hardcode (as in SyPet) specific polymorphism rules
  @SuppressWarnings("serial") public static Map<Class<?>, Set<Class<?>>> POLYMORPHISM_MAP = null;
  public static void loadSyPetPolymorphismMap() {
    if (POLYMORPHISM_MAP == null) {
      POLYMORPHISM_MAP = new HashMap<Class<?>, Set<Class<?>>>() {
        { // from SyPet's CONFIG.json
          put(java.awt.Shape.class,
              new HashSet<>(
                  Arrays.asList(java.awt.geom.Ellipse2D.class, java.awt.geom.Rectangle2D.class)));
          put(org.joda.time.ReadablePartial.class,
              new HashSet<>(Arrays.asList(org.joda.time.LocalDate.class)));
          put(org.joda.time.ReadableInstant.class,
              new HashSet<>(Arrays.asList(org.joda.time.DateTime.class)));
          put(org.w3c.dom.Node.class, new HashSet<>(Arrays.asList(org.w3c.dom.Element.class)));
          put(org.jsoup.nodes.Element.class,
              new HashSet<>(Arrays.asList(org.jsoup.nodes.Document.class)));
          put(java.io.Reader.class, new HashSet<>(Arrays.asList(java.io.StringReader.class)));
          put(org.joda.time.base.AbstractDateTime.class,
              new HashSet<>(Arrays.asList(org.joda.time.DateTime.class)));
          put(java.lang.Object.class, new HashSet<>(Arrays.asList(org.w3c.dom.Document.class)));
          put(org.joda.time.field.AbstractReadableInstantFieldProperty.class,
              new HashSet<>(Arrays.asList(org.joda.time.DateTime.Property.class)));
          put(org.apache.commons.math.linear.SingularValueDecomposition.class,
  ...
  public static final int MAX_LOOP_ITERATIONS =
      100; // Max number of total loop iterations (across all loops)
  public static final int MAX_SINGLE_LOOP_ITERATIONS =
      25; // Max number of consecutive iterations of a single loop
  ...
  public static final int MAX_COLLECTION_SIZE =
      100; // Doubles memory usage: list.addAll(list) and list.add(list.toString())
  ...
  public static final int MIN_LOOP_COND_SIZE =
      1; // MIN_LOOP_COND_SIZE <= (loop condition size) <= MAX_LOOP_COND_SIZE
  public static final int MAX_LOOP_COND_SIZE = 15;
  public static final int MIN_IF_COND_SIZE =
      1; // MIN_IF_COND_SIZE <= (if condition size) <= MAX_IF_COND_SIZE
  public static final int MAX_IF_COND_SIZE = 15;
  public static final int MAX_ANGELIC_CONDITIONS = 4;
  public static final int MAX_LINE_SIZE = 15; // applies to single-line statements, control
                                              // structure conditions, and the return expression
  ...
  public static final int SIMILAR_NEW_EXTRA_SIZE =
      8; // Without this, fragments wouldn't ever "grow"

  // Generating expressions to replace angelic conditions
  public static final int MAX_RESOLVE_COND_SIZE = 20;

  // Maximum number of programs to remember (for duplicate-checking), about 200 MB per million
  // elements
  ...
  public static boolean USE_SIMPLE_NAME =
      false; // "Point2D.Double" (true) or "java.awt.geom.Point2D.Double" (false)
}
